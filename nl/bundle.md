...
**Optie B: hele bundle in één.** [`nl/bundle.md`](https://raw.githubusercontent.com/joostliebregts/thoughtful-social-ai/main/nl/bundle.md) bevat alle vier docs samen — open, kopieer alles, plak in AI. *(Dit document.)*
...
## Wat staat in elke doc

**`nl/SOCIAL-AI-PRINCIPES.md`: 15 principes in 7 clusters.**
Fundamenteel (wijsheid in het systeem, vertrouwen, augmentation, taal). Grondhouding (bedachtzaamheid, frustratie als brandstof, kaart vs. gebied). Werkwijze (intentie, herkenning, tempo, iteratie, timing). Oplettend (formalisering kan vernietigen wat werkt). Data-eigenaarschap (data is van de mensen zelf). Anti-decontextualisatie (context maakt patroon). En de drie AI-waardeniveaus die in alle andere docs terugkomen.

**`nl/PROMPT-BEST-PRACTICES.md`: concrete vertaling naar prompt-ontwerp.**
Vier facetten als kompas. Vier kernbegrenzingen die elke transcript-prompt verdient. Drie AI-waardeniveaus (Snel, Doordacht, Diep). Plus technieken: multi-pass review, DIRECT vs INFERENCE-markeren, dubbele zekerheidsscoring, quote-density, transcript-artifacts handling. Geschreven om naast een sessie te leggen wanneer je de prompt bouwt.

**`nl/OwnershipPrinciples.md`: waar eigenaarschap vandaan komt.**
De psychologische grammatica (locus of control, self-efficacy, articulatie). Taalprincipes: authentieke stem behouden, context respecteren. Scoring-methodiek om eigenaarschap in transcripten te herkennen en te volgen. Voorbeelden van wat eigenaarschap wel en niet toont. Ontstaan via het Doesburg-traject, getoetst in volgende projecten.

**`nl/BottomUpPrinciples.md`: bottom-up systeemtransformatie.**
Zeven kernideeën uit het werkveld: wijsheid in het systeem, eigenaarschap als voorwaarde, organisch proces, energie als motor, dialoog als instrument, verschoven leiderschap, voorbereiding als grond. Plus twee stappenplannen: een voor beginners die net starten, een voor gevorderden die al in een traject zitten.
...
**Brug naar fundament:** deze bundle staat op een laag bottom-up praktijk-theorie die zelf niet hier woont. Voor wie dieper wil graven (organisch proces, leiderschap-verschuiving, vruchtbare grond als voorwaarde): zie [Bottom-up](./BottomUpPrinciples.md). Deze synthese is werk-georiënteerd; de fundamenten zitten een laag dieper.
...
*"Eigenaarschap komt van binnenuit. Het is niet iets dat wij toekennen, maar iets dat we herkennen en respecteren."* (zie [Eigenaarschap](./OwnershipPrinciples.md))
...
Raakt principe 4 en alle uitnodigings-werk. Voor de bredere theorie achter deze vier-lagen-lens: zie [Eigenaarschap](./OwnershipPrinciples.md).
...
- **Stille positieven binnen een negatieve stroom**: Een positief moment dat tegelijk speelt met een grote negatieve uitspraak in dezelfde minuut. De analyse pakt eerst het luide negatieve; het stille positieve gaat mee in dezelfde lezing.
  *Voorbeeld:* in een plenair na een breakout stonden 3 stemmen die zeiden "laat AI het maar doen" naast een bekende pushback-cascade. Default prompt zag alleen de cascade.

- **Stille positieven vlakbij een uitgebreid probleem**: Een werkend proces dat één zin verderop staat in een lange beschrijving van wat er misging. Aandacht zit op het probleem; het werkende patroon ernaast wordt overgeslagen.
  *Voorbeeld:* in een lang gesprek noemde één deelnemer in één zin een werkende directors-feedback-loop, ingebed in 2 uur over wat misging. Geen van de gemengde prompts pakte deze zin op.

- **Stille positieven verspreid over meerdere stukken**: Een positief patroon dat alleen zichtbaar wordt als je drie of vier verschillende stukken van één document samen leest. Elk los stuk is te klein.
  *Voorbeeld:* een drie-laags pre-check-systeem is alleen herkenbaar door drie secties samen te lezen. Default prompt vond ze individueel, niet als samenhang.
...
**Praktische implicatie:** zie [Prompts](./PROMPT-BEST-PRACTICES.md) voor concrete 3-pass setup.
...
Voor de volledige uitwerking: zie [Prompts](./PROMPT-BEST-PRACTICES.md).
...
- [Prompts](./PROMPT-BEST-PRACTICES.md): concrete vertaling naar prompt-ontwerp
...
**This is NOT**: AI replacing human judgment about ownership
**This IS**: AI helping humans articulate and systematize their existing expertise
